...
pub fn from_tool_use(tool_use: &ToolUse) -> Result<Action, ActionError> {
    ...
    match tool_use.name.as_str() {
        ...
        "drag" => {
            let start_x = get_i32(input, "start_x")?;
            let start_y = get_i32(input, "start_y")?;
...
mod tests {
    ...
    #[test]
    fn test_from_tool_use_wait_default_duration() {
        let tu = ToolUse {
    ...
    fn test_from_tool_use_wait_for_element_no_timeout() {
        let tu = ToolUse {
            id: "t_wfe2".to_string(),
            name: "wait_for_element".to_string(),
            input: json!({"description": "loading spinner"}),
        };
        let action = from_tool_use(&tu).unwrap();
        match action {
            Action::WaitForElement { description, timeout_ms } => {
                assert_eq!(description, "loading spinner");
    ...
    #[test]
    fn test_from_tool_use_batch_empty() {
        let tu = ToolUse {
            id: "t_batch2".to_string(),
            name: "batch".to_string(),
            input: json!({"actions": []}),
        };
        let action = from_tool_use(&tu).unwrap();
