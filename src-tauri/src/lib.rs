...
mod tests {
    use super::*;
    use crate::config::{GlmConfig, ProvidersConfig};

    /// Helper to create a Config with only GLM configured
    fn config_with_glm() -> Config {
        let mut config = Config::default();
        config.providers.glm = Some(GlmConfig {
    ...
    /// Helper to create a Config with no providers configured
    fn config_without_glm() -> Config {
        let mut config = Config::default();
        config.providers.glm = None;
        config
    }

    #[test]
    fn test_create_glm_provider_when_configured() {
        let config = config_with_glm();
        let result = create_provider_from_config("glm", &config);
        assert!(result.is_ok(), "Should return Ok when GLM is configured");
        let provider = result.unwrap();
        assert_eq!(provider.name(), "glm", "Provider name should be 'glm'");
    }

    #[test]
    fn test_create_glm_provider_when_not_configured() {
        let config = config_without_glm();
        let result = create_provider_from_config("glm", &config);
        assert!(result.is_err(), "Should return Err when GLM is not configured");
        assert_eq!(
            result.unwrap_err(),
            "GLM not configured",
            "Error message should be 'GLM not configured'"
        );
    }
    ...
    fn test_create_provider_unknown_returns_error() {
        ...
        let result = create_provider_from_config("nonexistent", &config);
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "Unknown provider: nonexistent");
    }

    #[test]
    fn test_create_provider_all_known_names_handled() {
        // Verify that all provider names that should be handled are recognized
        // (i.e., they don't return "Unknown provider" error)
        let mut config = Config::default();
        ...
        for name in known_providers {
            let result = create_provider_from_config(name, &config);
            assert!(
                result.is_err(),
                "Provider '{}' should error when not configured",
                name
            );
            // The error should be "X not configured", NOT "Unknown provider: X"
            let err = result.unwrap_err();
            assert!(
                !err.starts_with("Unknown provider"),
                "Provider '{}' should be handled in match, got: {}",
                name,
                err
            );
            assert!(
                err.contains("not configured"),
                "Provider '{}' error should say 'not configured', got: {}",
                name,
                err
            );
        }
