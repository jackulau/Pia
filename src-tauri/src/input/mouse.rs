...
impl MouseController {
        ...
        let enigo = THREAD_MOUSE_ENIGO.with(|cell| {
            ...
            if let Some(existing) = opt.take() {
                ...
                Enigo::new(&Settings::default()).map_err(|e| {
                    ...
                    MouseError::InitError(error_msg)
                })
            }
        })?;
    ...
    /// Check whether the current display server supports mouse input simulation.
    /// Returns Ok(()) if supported, or Err with a descriptive message if not.
    pub fn check_wayland_compatibility() -> Result<(), MouseError> {
        let compat = crate::platform::check_display_compatibility();
        if !compat.input_supported {
            return Err(MouseError::WaylandError(format!(
                "Mouse input not supported on {}. {}",
