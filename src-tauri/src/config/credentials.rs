...
fn detect_glm(file_sources: &HashMap<String, FileSourceEntry>) -> Option<DetectedCredential> {
    // 1. Check env vars and file sources
...
fn scan_config_files(provider: &str) -> Vec<(String, String)> {
    ...
    match provider {
        ...
        "glm" => {
            ...
            let litellm_path = home.join(".config/litellm/config.yaml");
            if let Some(key) = read_key_from_yaml(&litellm_path, "api_key") {
                // Only include if it looks like a GLM key (JWT-like with '.') and not another provider
...
mod tests {
    ...
    // --- GLM config file scanning tests ---

    #[test]
    fn test_scan_config_files_glm_litellm_valid_key() {
        // Create a temp dir to simulate ~/.config/litellm/
        let dir = tempfile::tempdir().unwrap();
        let litellm_dir = dir.path().join(".config/litellm");
        fs::create_dir_all(&litellm_dir).unwrap();
        let config_path = litellm_dir.join("config.yaml");
        fs::write(&config_path, "api_key: abc123def.ghijklmnop456\nmodel: glm-4v\n").unwrap();

        // We can't easily override dirs::home_dir(), so test read_key_from_yaml directly
        let key = read_key_from_yaml(&config_path, "api_key");
        assert_eq!(key, Some("abc123def.ghijklmnop456".to_string()));

        // Verify the key passes GLM format validation
        let key_val = key.unwrap();
        assert!(key_val.contains('.'));
        assert!(!key_val.starts_with("sk-"));
        assert!(validate_key_format("glm", &key_val));
    }

    #[test]
    fn test_scan_config_files_glm_rejects_anthropic_key_in_litellm() {
        // A LiteLLM config with an Anthropic key should not be picked up for GLM
        let dir = tempfile::tempdir().unwrap();
        let config_path = dir.path().join("config.yaml");
        fs::write(&config_path, "api_key: sk-ant-abcdef1234567890\n").unwrap();

        let key = read_key_from_yaml(&config_path, "api_key").unwrap();
        // This key starts with "sk-", so the GLM filter should reject it
        assert!(key.starts_with("sk-"));
    }

    #[test]
    fn test_scan_config_files_glm_rejects_openai_key_in_litellm() {
        // A LiteLLM config with an OpenAI key should not be picked up for GLM
        let dir = tempfile::tempdir().unwrap();
        let config_path = dir.path().join("config.yaml");
        fs::write(&config_path, "api_key: sk-proj-abcdef1234567890\n").unwrap();
    ...
    #[test]
    fn test_scan_config_files_glm_rejects_key_without_dot() {
        let dir = tempfile::tempdir().unwrap();
        let config_path = dir.path().join("config.yaml");
        fs::write(&config_path, "api_key: nodotinthiskey1234567\n").unwrap();

        let key = read_key_from_yaml(&config_path, "api_key").unwrap();
        // Key has no dot, so it should not be accepted as GLM
        assert!(!key.contains('.'));
    }

    #[test]
    fn test_scan_config_files_glm_empty_config() {
        let dir = tempfile::tempdir().unwrap();
        let config_path = dir.path().join("config.yaml");
        fs::write(&config_path, "model: glm-4v\n").unwrap();

        // No api_key field
        assert_eq!(read_key_from_yaml(&config_path, "api_key"), None);
    }

    #[test]
    fn test_detect_glm_config_file_fallback() {
        // When no env vars or file sources are available, detect_glm should
        // attempt config file scanning. We can't easily control the file system
        // for the full integration, so test that the function doesn't panic
        // with empty sources.
        if env::var("GLM_API_KEY").is_err()
            && env::var("ZHIPUAI_API_KEY").is_err()
            && env::var("GLM_KEY").is_err()
        {
            let sources = HashMap::new();
            // This will attempt scan_config_files("glm") as fallback
            let result = detect_glm(&sources);
            // Result depends on whether ~/.config/litellm/config.yaml exists
            // but should not panic either way
            if let Some(cred) = result {
                assert_eq!(cred.provider, "glm");
                assert!(cred.api_key.contains('.'));
                assert!(cred.source.starts_with("config:"));
            }
        }
    }
